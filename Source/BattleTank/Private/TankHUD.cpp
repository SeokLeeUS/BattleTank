...
FVector2D ATankHUD::ConvertWorldLocationToLocal(AActor * ActorToPlace)
{
...
FVector2D ATankHUD::GetRadarCenterPosition()
{
...
void ATankHUD::DrawPlayerInRadar()
{
	FVector2D RadarCenter = GetRadarCenterPosition();
...
void ATankHUD::PerformRadarRaycast()
{
	APawn* Player = UGameplayStatics::GetPlayerPawn(GetWorld(), 0);
	...
	if (Player)
	{
		...
		for (auto It : HitResults)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Radar Detected %2"), *CurrentActor->GetName())
...
void ATankHUD::DrawRaycastedActors()
{
	FVector2D RadarCenter = GetRadarCenterPosition();
